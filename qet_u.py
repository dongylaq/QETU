"""
    Hadamard Block-encoding implemented using IBM's Qiskit.
...
class HadamardBlockEncoding(object):
    """Implement the Hadamard block-encoding"""
    __n_be_qubit = 1

    def __init__(self):
        pass

    def build_from_unitary(
            self, 
            qc: "QuantumCircuit", 
            angle: float = 0
        ) -> None:
...
def measure_energy_VQEType(
    ...
    job = execute(
        circ_comp, 
        backend, 
        shots = meas_shots_comp, 
        noise_model=noise_model_comp,
    ...
    job = execute(
        circ_had, 
        backend, 
        shots = meas_shots_had,
        noise_model=noise_model_had,
        optimization_level = 0
    )
    res_had = job.result()
