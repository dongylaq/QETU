...
class QSPCircuit(object):
    ...
    def __init__(self, n_sig_qubit, n_be_qubit, n_sys_qubit):
        ...
        self.qr = QuantumRegister(self.n_tot_qubit, 'q')
        self.cr = ClassicalRegister(self.n_tot_qubit, 'c')

        self.qc = QuantumCircuit(self.qr, self.cr, name='QSP')
    ...
    def build_circuit(self, be_qc, phi_seq, realpart=True, measure=False, init_prepare=None, measure_full=False):
        """Build a QSP circuit.
        ...
        Returns:
            None. This function provides a circuit in self.qc .
        """
        assert (measure and measure_full) is not True

        if measure:
            self.cr = ClassicalRegister(self.n_sig_qubit+self.n_be_qubit)
        if measure_full:
            self.cr = ClassicalRegister(self.n_tot_qubit)
        qr = self.qr
        cr = self.cr
        ...
        if init_prepare is not None:
            if isinstance(init_prepare, QuantumCircuit):
                assert init_prepare.num_qubits + 2 == self.n_tot_qubit
                self.qc.append(init_prepare.to_instruction(), qr[2:])
        ...
        self.qc.append(qc_crot.to_instruction(), qr[0:self.n_sig_qubit+self.n_be_qubit])
        self.qc.barrier()
        for phi in reversed(phi_seq[:-1]):
            if not dag:
                self.qc.append(be_qc.to_instruction(), qr[1:])
            else:
                self.qc.append(be_qc_dag.to_instruction(), qr[1:])
            self.qc.barrier()
        ...
        if measure:
            self.qc.measure(qr[:self.n_sig_qubit+self.n_be_qubit], cr)

        if measure_full:
            self.qc.measure(qr, cr)
...
class Hermitian_BlockEncoding(object):
    ...
    def build_random_circuit(self, n_depth, coupling_map=None, basis_gates=None, prob_one_q_op=0.5):
        ...
        qc = random_circuit(self.n_tot_qubit-1, n_depth,
                coupling_map=coupling_map, basis_gates=basis_gates, prob_one_q_op=prob_one_q_op)
        qc.name = 'UA_0  '
        self.qsp.build_circuit(qc, self.phi_seq, realpart=True, measure=False)
    ...
    def build_canonical_hracbem(self, n_depth, coupling_map=None, basis_gates=None, prob_one_q_op=0.5):
        ...
        if hasattr(self, 'qc'):
            ...
            self.qc = QuantumCircuit(self.qr, name='UH  ')
        # n_tot_qubit-1: exclude the first be_qubit as sig_qubit in quadratic QSVT
        qc = random_circuit(self.n_tot_qubit-1, n_depth,
                coupling_map=coupling_map, basis_gates=basis_gates, prob_one_q_op=prob_one_q_op)
        qc.name = 'UA_0  '
